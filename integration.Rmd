...
curve(dgamma(x, shape = prior.shape, scale = prior.scale), add = TRUE,
      lty = 2)
legend("topright", legend = c("Posterior Density", "Prior Density", "Posterior Mean", "Posterior Mode"), lty = c(1, 2, 1, 1), lwd = c(3, 1, 1, 1), col = c(1, 1, 3, 2), bty = "n")
curve(lapprox, 0.001, 20, n = 1000, add = TRUE, col = 2, lwd = 2)
abline(v = (y + prior.shape) * (1 / (length(y) + 1 / prior.scale)),
       col = "steelblue")
abline(v = pmode, col = 2)
```

The solid red line is the Laplace approximation and we can see that in the neighborhood of the mode, the approximation is reasonable. However, as we move farther away from the mode, the tail of the Gamma is heavier on the right. 
